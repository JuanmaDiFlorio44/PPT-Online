...
//Una vez subido a render poner direccion aqui
...
const state = {
  ...
  conectRoomRt() {
    ...
    onValue(chatRoomRef, (snapshot) => {
      ...
      // Verifica si `history` existe antes de asignarlo
      currentState.history = dbRealTime && dbRealTime.history ? dbRealTime.history : [];
  
      // Verifica si `playerUno` y `playerDos` existen en dbRealTime
      currentState.currentGame = {
        playerUno: dbRealTime && dbRealTime.playerUno ? dbRealTime.playerUno : null,
        playerDos: dbRealTime && dbRealTime.playerDos ? dbRealTime.playerDos : null,
      };
  ...
  playersStart() {
    ...
    // Verificar que cs, cs.currentGame, playerUno y playerDos no sean undefined
    if (
        cs &&
        cs.currentGame &&
        cs.currentGame.playerUno &&
        cs.currentGame.playerDos &&
